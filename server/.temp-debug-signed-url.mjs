...
try {
  const userResult = await pool.query('select id, role, status from users where status = $1 limit 1', ['ACTIVE']);
  const user = userResult.rows[0];
  console.log('USER', user);
  const token = jwt.sign({ id: user.id, role: user.role, status: user.status }, process.env.JWT_SECRET, { expiresIn: '1h' });
  ...
  console.log('SIGNED_URL_RES', signedUrlRes.data);
